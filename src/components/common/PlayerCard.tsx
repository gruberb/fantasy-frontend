...
const PlayerCard = ({
  ...
  const baseStyle =
    "bg-white rounded-lg border border-gray-200 shadow-sm flex items-center justify-between";
      ...
      <div className="flex items-center w-full min-w-0 overflow-hidden">
        ...
        {player.imageUrl ? (
          <img
            ...
            className={`${imageSize} rounded-full mr-3 border border-gray-100 flex-shrink-0`}
          />
        ) : (
          <div
            className={`${imageSize} bg-gray-200 rounded-full flex items-center justify-center mr-3 flex-shrink-0`}
          >
        ...
        {showPoints && (
          <div
            className={`${valueTextSize} font-bold ${valuePadding} rounded-md flex-shrink-0 ml-auto ${
              (player.points || 0) > 0
